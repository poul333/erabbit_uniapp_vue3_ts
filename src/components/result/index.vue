...
<template>
  ...
  <view class="result">
    <view class="tabs">
      <view :class="{ active: tabIndex === 0 }" class="item" @tap="doSelect">
        <text class="label">{{ optionValue }}</text>
        <text :class="[selecting ? 'icon-up' : 'icon-down']"></text>
      </view>
      <view :class="{ active: tabIndex === 1 }" class="item" @tap="byPrice">
        <text class="label">价格</text>
        <text :class="sortIcon"></text>
      </view>
      <view :class="{ active: tabIndex === 2 }" class="item" @tap="bySales">
        <text class="label">销量</text>
      </view>
      <view class="item" @tap="goFilter">
    ...
    <!-- 下拉框 -->
    <view class="mask" v-show="selecting" @tap="changeOption">
      <view class="select">
        <view
          class="option"
          data-option="最新"
